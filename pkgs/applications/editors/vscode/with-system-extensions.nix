...
/*
  ...
  $ rm -rf "$PWD/result-build-vscode-w-sys" &&  out="$PWD/result-build-vscode-w-sys" genericBuild
  ```

  ```bash
  $ nix-build -I nixpkgs=. -E 'with (import <nixpkgs> {}); vscode-with-system-extensions.override {vscodeExtensions = with vscode-extensions; [bbenoist.Nix ms-vsliveshare.vsliveshare]; }' --out-link ./result-vscode-w-sys
  ```
*/
...
runCommand "${wrappedPkgName}-with-system-extensions-${wrappedPkgVersion}" {
  ...
  # Not required.
  # symlink_reinstall_as_real_mut_dir "$out/lib/vscode/resources/app/node_modules.asar.unpacked"
